'use strict';(function(d){function x(a,c){var f="",e="",p=a.find("input"),h=b.session.getTextRange(b.getSelectionRange());0<h.trim().length&&p.eq(0).val(h);a.find(".modal-footer button.btn-success").on("click.codeparlMarkdown",function(){f=p.eq(0).val();e=p.eq(1).val();0<e.length&&0<f.length?(a.modal("hide"),c?l.insertSnippet(b,"["+f+"]("+e+")"):l.insertSnippet(b,"!["+f+"]("+e+")")):y(a)})}function m(a,c){var f=b.session.getTextRange(b.getSelectionRange())||"",e=b.find(c,{wrap:!0,caseSensitive:!0,
wholeWord:!0,regExp:!1,preventScroll:!0});null!==e?b.session.replace(e,c):0===b.getCursorPosition().column?(b.navigateLineStart(),"``"===a?b.insert("`"+c+"` "):b.insert(a+c+" "),b.navigateLineEnd()):0<f.trim().length?(e=b.getSelectionRange(),"``"===a?(b.insert("`"+f+"` "),b.session.replace(e,"`"+f+"` ")):b.session.replace(e,a+f)):("``"===a?b.insert("`"+c+"` "):b.insert(a+c+" "),b.navigateLineEnd());b.find(c);b.focus()}function z(a){"function"!==d.type(d().modal)&&d.error("bootstrap.min.js is required for this plugin");
...
function(){d(this).remove()});c.find(".modal-body").html(a.body);C(c);return c}function C(a){a.on("click.codeparlMarkdown",function(c){d(c.target).is(".modal-dialog *")||y(a)})}function A(a,c){a=a?a:"Text";c=c?c:"URL";var f=d("<input>").attr({type:"text",placeholder:"Enter "+a}).prop("required",!0).addClass("form-control form-control-sm");f=d("<label>").addClass("w-100").text(a+":").append(f);a=d("<input>").attr({type:c,placeholder:"Enter "+c}).prop("required",!0).addClass("form-control form-control-sm");
a=d("<label>").addClass("w-100").text(c+":").append(a);return d("<div>").append(f).append(a)}function B(a,c){a?(q.show(),n.hide(),d(".cpme-btn-normal").prop("disabled",!1).removeClass("disabled")):(q.hide(),n.show(),a=t.makeHtml(b.session.getValue()),g.onPreview(u(a)),n.html(u(a)),d(".cpme-btn-normal").prop("disabled",!0).addClass("disabled"));c.addClass("active");c.siblings("button").removeClass("active")}function D(a){a.on("change.codeparlMarkdown",function(){g.onFileBrowse(a,b)})}function u(a){if(!g.allowScript)return a.replace("<script>",
"&lt;script&gt;")}function y(a){a.find(".modal-content").addClass(" animated fast headShake");setTimeout(function(){a.find(".modal-content").removeClass("animated fast headShake")},1E3)}function E(){k.find(".cpme-toolbar .cpme-btn").each(function(){d(this).on("click.codeparlMarkdown",function(a){a.preventDefault();a=d(this);var c=a.data("action");d("[data-original-title").tooltip("hide");switch(c){case "preview":B(!1,a);break;case "edit":B(!0,a);break;case "browse":a=a.next(".cpme-file-browser");
...
"type here");break;case "code":m("``","enter code here");break;case "bold":b.execCommand("bold");break;case "italic":b.execCommand("italic");break;case "link":b.execCommand("link");break;case "image":a=z({title:"Enter Image URL",body:A("Alt Text")}).modal("show"),x(a,!1)}})})}function F(){var a=d("<div>").addClass("cpme-toolbar border-bottom p-1 ").css({backgroundColor:g.toolbar.bg});for(const p in g.toolbar.buttonGroups){var c=g.toolbar.buttonGroups[p];var f=a;f=d("<div>").addClass("btn-group cpme-btn-group   mr-1").appendTo(f);
