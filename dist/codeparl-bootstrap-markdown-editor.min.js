...
a=O(f);a.prependTo(l);a.append(b("<div>").addClass("clearfix"));f.help.show&&P().insertAfter(a);k=b("<div>").addClass("cpme-preview w-100 markdown-body overflow-auto p-2 bg-white border").hide().appendTo(l).css({height:f.editor.editorHeight+"px"});Q(f);N(f);return this},markdownContent:function(a){if(a)d.session.setValue(a),d.clearSelection();else return convertToLink(d.getValue())},htmlContent:function(){return r.makeHtml(convertToLink(d.session.getValue()))},preview:function(a,c){a&&(c=b.extend(!0,
